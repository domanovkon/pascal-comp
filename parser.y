...
functions : function-declaration { $$ = new NBlock(); typeid($1).name(); $$->statements.push_back($1); std::cout << "function\n "; }
                | functions function-declaration { $1->statements.push_back($2); std::cout << "function\n ";}
                ;


function-declaration : function-heading function-body { $$ = new NFunctionDeclaration(*$1, *$2); std::cout << "function-declaration\n "; }           
                |  function-heading T_VAR declarations-list function-body { $$ = new NFunctionDeclaration(*$1, *$4, $3); std::cout << "function-declaration\n "; }
                ;
...
function-statement : function-identifier T_OPAREN actual-parameter-list T_CPAREN { $$ = new NMethodCall(*$1, *$3); std::cout << "function-call-statement with PARAMS\n";}
                ;
...
digit-sequence : unsigned-digit-sequence {$$ = $1;}
                | sign unsigned-digit-sequence %prec UMINUS { /* Для устранения конфликта и выбора приоритета */ if($1 == T_MINUS) {$$=-1*$2; std::cout << "set Sign "<<$$<<"\n";} else { $$ = $2; } }
                ;
